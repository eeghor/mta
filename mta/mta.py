...
class MTA:
    ...
    def get_generated_conversions(
        self, max_subset_size: Optional[int] = None
    ) -> "MTA":
        """Build conversion totals by exact unique channel set."""
        ...
        for ch_list, convs, nulls in zip(
            ...
            self.cc[channels][self.CONV] += convs
            self.cc[channels][self.NULL] += nulls

        return self

    def v(self, coalition: Tuple[Any, ...]) -> float:
        """
        Total conversions generated by observed journeys whose channels are a
        subset of the coalition.
        """

        coalition = self.ordered_tuple(coalition)
        s = len(coalition)
    ...
    def shapley(
        ...
        Args:
            max_coalition_size: Maximum size of coalitions to consider before
                adding the evaluated channel. None evaluates all coalitions.
            normalize: Whether to normalize results
        """
        if max_coalition_size is not None and max_coalition_size < 0:
            raise ValueError("max_coalition_size must be non-negative or None")

        self.get_generated_conversions()
        ...
        for channel in self.channels:
            ...
            max_subset_size = (
                ...
                else min(max_coalition_size, len(other_channels))
            )

            # Consider all subset sizes up to max_coalition_size
            for subset_size in range(0, max_subset_size + 1):
